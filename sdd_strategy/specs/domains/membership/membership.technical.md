...
| Where a member's plan lives | `Subscription.Plan` is the authority; `User.Role` mirrors it, driven by `MirrorPlanOntoUserRoleHandler` | The role already doubled as the plan before this domain existed, and the access token carries it. Making the mirror event-driven means no plan-changing path has to remember it. Nothing is authorised on the plan portion of the role — every plan gate goes through `IEntitlementProvider`, which reads the subscription — so a role claim stale for the remaining life of an access token cannot grant anything unpaid for | Removing the tiers from `UserRole` — rejected for this stage: it would rewrite token claims, policies, seeds and the frontend for no behavioural gain. Recorded as debt. Letting each side write independently — rejected: two writers, one fact |

...
| `GLOBAL-022` | **Make the create endpoints answer one shape** | ⬜ | — | — | Raised 2026-08-16 during Stage 6. Three creates answer three different bodies: `POST /admin/catalog/books` a bare `Guid`, `POST /network/libraries` an `{ id }`, and `POST /network/admins` an `{ invitationId }`. Not a defect on its own, but it produced one immediately — the Stage 6 client read `.id` from all three and silently got `undefined` from two, which only surfaced when the wizard was exercised against the running system. Worth settling before more clients are written |

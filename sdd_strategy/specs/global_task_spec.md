...
| `GLOBAL-027` | **Member defaults do not follow a member between devices** | ⬜ | — | — | The three defaults on Settings — delivery, returns, purchase fulfilment — are held in browser storage, as the prototype holds them. They are proposals, not decisions, and the binding choice is validated per reservation and per order. Giving them persistence needs a home for member preferences that no bounded context obviously owns: `DeliveryMethod` belongs to `reservations` and `OrderFulfilment` to `store`, so neither can hold all three |
| `GLOBAL-028` | **No way to promote an existing card to default** | ⬜ | — | — | The prototype's payment-method card offers **Make default** beside **Remove**. There is no endpoint: `AddPaymentMethod` takes `MakePrimary`, and nothing promotes a card already on file. The control is left out rather than shown and refused. Needs a command, or a decision that the prototype's affordance is dropped |
| `GLOBAL-029` | **Payment history has no method or receipt** | ⬜ | — | — | The prototype's payment history table carries five columns; a `LedgerEntry` supports four. It records neither which card paid nor a receipt number, so those two columns are absent rather than filled with something invented. Adding them means putting the payment method and a receipt reference on the ledger entry at the point the payment is taken |
| `GLOBAL-030` | **Review comments are stored and never shown** | ⬜ | — | — | `BR-CAT-027` lets a member attach a comment to their rating, and the API returns every review with its text. No screen displays them: the prototype shows only the aggregate on a card and in the book panel, and its own state (`s.reviews`) is read solely to label the loan row and to seed the rating dialog. The read-only list this build had on the book panel was removed for that reason. Either the comments earn a surface, or `BR-CAT-027` should stop collecting them |
...
| `GLOBAL-023` | **Set the recommendation rate limit (`BR-REC-011`)** | ⬜ | — | — | Raised 2026-08-16 while authoring the `recommendations` specs. The rule requires a limit and names no figure. Proposed: one regeneration per member per hour, with reads always served from cache. Blocks `REC-015` only — reading recommendations is unaffected |

...
public sealed class ReturnBookFromRepairCommandHandler(
    ICatalogUnitOfWork catalog,
    IAuditUnitOfWork audit,
    ICurrentUser currentUser,
    IDateTimeProvider clock) : ICommandHandler<ReturnBookFromRepairCommand>
{
    public async Task<Result> Handle(
        ReturnBookFromRepairCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = book.ReturnFromRepair(clock.UtcNow);
        ...
        await audit.Entries.AddAsync(
            AuditEntry.Record(
                "catalog.book_returned_from_repair",
                clock.UtcNow,
